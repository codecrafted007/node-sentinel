...
```sh
./scripts/docker-build.sh binaries     # -> bin/linux_amd64/{agent,controller,sentinelctl} + bin/linux_arm64/...
./scripts/docker-build.sh image        # -> node-sentinel:dev, host arch, loaded into docker
./scripts/docker-build.sh image --push -t <registry>/node-sentinel:<tag>   # multi-arch (amd64+arm64) manifest
```

This works because `internal/ebpf/bpf/vmlinux.h` is **committed** (not host-dumped): CO-RE relocates field offsets at load time, so one checked-in header serves every kernel/arch. Regenerate it only when adding probes that read new kernel structs (`make vmlinux`, or `./scripts/docker-build.sh vmlinux`).
...
- Narrative docs live in `docs/` (scripts in `scripts/`). `docs/CONCEPTS.md` explains what the system does and how it decides (offender/victim, baseline, confidence) in plain-English analogies — it marks each idea ✅ built vs 🔜 planned, so keep those tags accurate as features land.
- `docs/HOW.md` is the onboarding explainer for how the eBPF probe is compiled, embedded (bpf2go + `go:embed`), loaded, and attached — point new contributors there. `docs/ARCHITECTURE.md` has the diagrams; `docs/HELPERCOMMANDS.md` the command reference; `docs/DETECTION-DEMO.md` a live-cluster walkthrough.
- See `docs/PROGRESS.md` for a running log of completed work.
