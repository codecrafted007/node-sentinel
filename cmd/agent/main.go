...
func main() {
	...
	flag.IntVar(&cfg.RetransWarn, "retrans-warn", cfg.RetransWarn, "min absolute TCP retransmits/interval for a network victim (secondary gate)")
	flag.Float64Var(&cfg.RetransRateWarn, "retrans-rate-warn", cfg.RetransRateWarn, "TCP retransmit rate (retransmits/segs) a pod must exceed to count as a network victim")
